...
function Header() {
    const {state:{quantity},productDispatch} = CartState();
    const {user,isLogin,setIsLogin} = AuthState();
          ...
          <div className="nav">
            ...
            <div className="nav-wrap flx1 flx-right">
              ...
              {isLogin===false ? (
                ...
                <div className="nav-item hide">
                  <div className="wrap-avatar" onClick={() => setIsClickAvatar(!isClickAvatar)}>
                    <img
                      src={user.avatar}
                      alt="avatar"
                    ...
                    <div className="avatar-dropdown t-56" onClick={(e) => e.stopPropagation()} style={isClickAvatar === true ? { display: "block" } : { display: "none" }}>
                      <div className="avatar-dropdown-list">
                        <Link to={user.name==='Admin'?'/login/admin':'/'} className="avatar-dropdown-item avatar-link">
                          <div className="item-icon">
                            {user.name!=='Admin'?<svg width="18px" height="18px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                          ...
                          <span className="info-name">{user.name==='Admin'?'Dashboard':'Info detail'}</span>
                        </Link>
                        <div className="avatar-dropdown-item" onClick={() => { window.location.reload(false);}}>
                          ...
                          <span className="info-name">Logout</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              >
                ...
                <Link to={user.name==='Admin'?'/login/admin':'/'} className="sidenav-link">{user.name==='Admin'?'Dashboard':'Info detail'}</Link>
                <Link to="/" className="sidenav-link" onClick={() => {setIsClick(false);setIsLogin(false)}}>Logout</Link>
                </>}
              </div>
            </div>
          </div>
